import { m as macro } from '@kitware/vtk.js/macros2';
...
function vtkImageMarchingCubes(publicAPI, model) {
	...
	publicAPI.getVoxelScalars = (i, j, k, slice, dims, origin, spacing, s) => {
		...
		for (let ii = 0; ii < 8; ++ii) {
			...
			/* <<<<<< added */
			// voxelScalars[ii] = (self.data_segm[ids[ii]] && 1) * self.data_orig[ids[ii]];
			/* >>>>>> */
		}
	};
	...
	>>>>>> <<<<<< added */
	publicAPI.produceTriangles = (cVal, i, j, k, extent, slice, dims, origin, spacing, scalars, points, colors, tris, normals) => {
	/* >>>>>> */
		...
		for (let idx = 0; voxelTris[idx] >= 0; idx += 3) {
			...
			for (let eid = 0; eid < 3; eid++) {
				...
				if (pId === undefined) {
					const t = (cVal - (voxelScalars[edgeVerts[0]] || 0)) / ((voxelScalars[edgeVerts[1]] || 0) - (voxelScalars[edgeVerts[0]] || 0));
					...
					/* <<<<<< added */
					// {
					// 	const xx0 = voxelScalars.slice(edgeVerts[0] * 1, (edgeVerts[0] + 1) * 1);
					// 	const xx1 = voxelScalars.slice(edgeVerts[1] * 1, (edgeVerts[1] + 1) * 1);
					// 	// LOG(xx0, xx1)
					...
					// 	const _min = global.min;
					// 	const _max = global.max

					// 	let greyscale = (global.data_orig[_index] - _min) / (_max - _min);
	...
	publicAPI.requestData = (inData, outData) => {
		...
		>>>>>> <<<<<< added */
		// for (let k = k_min || 0; k < (k_max || dims[2] - 1); ++k) {
		// 	for (let j = j_min || 0; j < (j_max || dims[1] - 1); ++j) {
		// 		for (let i = i_min || 0; i < (i_max || dims[0] - 1); ++i) {
