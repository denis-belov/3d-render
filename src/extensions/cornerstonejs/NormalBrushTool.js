...
function fillSphere(enabledElement, operationData, _inside = true, threshold = false) {
    ...
    if (threshold) {
        callback = ({ value, index, pointIJK, bounds }) => {
            ...
            if (isWithinThreshold(index, imageVolume, strategySpecificConfiguration)) {
                ...
                // segmentation.vtkOpenGLTexture.update3DFromRaw4(segmentIndex, ...pointIJK);
                /* <<<<<< added */
                // fillDataFor3d(index, segmentIndex);
                syncSegm(viewport, pointIJK, imageData, segmentIndex, slice_range);
                /* >>>>>> */
            }
        };
    }
    else {
        callback = ({ index, value, pointIJK, bounds }) => {
            if (segmentsLocked.includes(value)) {
                return;
            }
            scalarData[index] = segmentIndex;
    ...
    // if (viewport.dst_viewport)
    // {
    //     const volumeId =
    //         viewport.dst_viewport
    //             .getActors()
    //             .find(actor_desc => actor_desc !== viewport.dst_viewport.getDefaultActor())
    //             .referenceId;

    //     const [ minSlice, maxSlice ] = slice_range;
    //     const sliceArray = Array.from({ length: maxSlice - minSlice + 1 }, (v, k) => k + minSlice);
    //     triggerSegmentationDataModified(volumeId, sliceArray);
    // }
}
...
function fillCircle(enabledElement, operationData, threshold = false) {
    ...
    const { imageData, dimensions, spacing } = segmentationVolume;
    ...
    if (threshold) {
        callback = ({ value, index, pointIJK }) => {
            ...
            if (isWithinThreshold(index, imageVolume, strategySpecificConfiguration)) {
                ...
                /* <<<<<< added */
                // fillDataFor3d(index, segmentIndex);
                syncSegm(viewport, pointIJK, imageData, segmentIndex, segmentIndex);
                /* >>>>>> */
            }
        };
    }
...
import { resetElementCursor } from '@cornerstonejs/tools/dist/esm/cursors/elementCursor';
export default class NormalBrushTool extends BrushTool
{
    constructor
    (
        toolProps = {},
...
NormalBrushTool.toolName = 'NormalBrush';
