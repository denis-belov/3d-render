...
DOWNSTREAM_SERVER_URL = 'http://localhost:8080/upload'  # Опционально
...
async def handle_radiomics(request: Request) -> Response:
    ...
    try:
        ...
        if 'multipart/form-data' in content_type:
            ...
            mask_file = form_data.get('mask')

            if not image_file:
                raise ValueError("Отсутствует файл изображения")
            if not mask_file:
                raise ValueError("Отсутствует файл маски")
            ...
            if hasattr(mask_file, 'file'):
                mask_data = mask_file.file.read()
            elif hasattr(mask_file, 'read'):
                mask_data = mask_file.read()
            else:
                raise ValueError("Неверный формат данных маски")
