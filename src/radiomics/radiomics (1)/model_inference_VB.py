...
from pydicom.dataset import Dataset, FileDataset, FileMetaDataset
from pydicom.uid import generate_uid, ExplicitVRLittleEndian
#art_path = "art_radiomics_features_20251119_190235 — копия.csv"
#port_path = "port_radiomics_features_20251119_190235.csv"


def get_ai_result(art_path, port_path):
    out_path = "merged.csv"
    ...
    # ВЕСА МОДЕЛИ 
    PIPELINE_PATH = "112full_pipeline.pkl"

    # путь к файлу с тестовой таблицей 
    TEST_PATH = "merged.csv"
    DATA_PATH = "общая таблица (все кроме кровотечений и внешней валидации) (1) - общая таблица (все кроме кровотечений и внешней валидации) (1).csv.csv" # в финальной версии не будет
    ...
    # Заполняем средними из обучающей выборки (гарантированно числа)
    train_means_safe = train_means.fillna(0)
    X_new = X_new.fillna(train_means_safe)

    # На всякий случай — если остались NaN (например, в колонках, которых не было в train),
    # заполняем нулями (SVC после StandardScaler это переживёт)
    ...
    print('out_df["pred_label"]', out_df["pred_label"])

    res_text = ''
    if out_df["pred_label"][0] == 0:
        res_text = "псевдотуморозный панкреатит"
    if out_df["pred_label"][0] == 1:
        res_text = "протоковая аденокарцинома"
    return res_text
